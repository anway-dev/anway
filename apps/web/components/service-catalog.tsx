...
import { CATALOG_SERVICES, INCIDENTS, CatalogService, ServiceHealth } from "@/lib/mock";
...
type HealthFilter = "all" | ServiceHealth;

interface Props {
  onTriggerOrchestrator: (query: string, context: { title: string; source: string }) => void;
}

function DepGraph({ svc }: { svc: CatalogService }) {
  const callers = CATALOG_SERVICES.filter(s => svc.callers.includes(s.id));
  const deps    = CATALOG_SERVICES.filter(s => svc.dependencies.includes(s.id));
    ...
    <div style={{ position: "relative", height: `${CANVAS_H}px`, background: "#0e0e0e", border: "1px solid #1a1a1a", borderRadius: "8px", overflow: "hidden" }}>
        ...
        {callerPositions.map((pos, i) => {
          ...
          return (
            <path
              ...
              fill="none"
              stroke={HEALTH_COLOR[callers[i].health]}
              strokeWidth="1"
              strokeDasharray={callers[i].health === "healthy" ? "none" : "4 2"}
              opacity="0.4"
            />
          );
        })}
        {depPositions.map((pos, i) => {
          ...
          return (
            <path
              ...
              d={`M ${fromX} ${fromY} C ${mx} ${fromY}, ${mx} ${toY}, ${toX} ${toY}`}
              fill="none"
              stroke={HEALTH_COLOR[deps[i].health]}
              strokeWidth="1"
              strokeDasharray={deps[i].health === "healthy" ? "none" : "4 2"}
              opacity="0.4"
            />
          );
        })}
      ...
      {callers.map((c, i) => (
        <div
          ...
          style={{
            position: "absolute",
            left: callerPositions[i].x,
            top: callerPositions[i].y,
            width: BOX_W, height: BOX_H,
            background: "#111", border: `1px solid ${HEALTH_COLOR[c.health]}40`,
            borderRadius: "6px", display: "flex", alignItems: "center",
            padding: "0 8px", gap: "6px",
          }}
        >
          <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: HEALTH_COLOR[c.health], flexShrink: 0 }} />
      ...
      <div
        style={{
          ...
          width: BOX_W, height: BOX_H,
          background: "#0a150e", border: "1px solid rgba(16,185,129,0.5)",
          borderRadius: "6px", display: "flex", alignItems: "center",
          padding: "0 10px", gap: "6px",
          boxShadow: "0 0 12px rgba(16,185,129,0.15)",
        }}
      >
        <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: HEALTH_COLOR[svc.health], flexShrink: 0 }} />
      ...
      {deps.map((d, i) => (
        <div
          ...
          style={{
            position: "absolute",
            left: depPositions[i].x,
            top: depPositions[i].y,
            width: BOX_W, height: BOX_H,
            background: "#111", border: `1px solid ${HEALTH_COLOR[d.health]}40`,
            borderRadius: "6px", display: "flex", alignItems: "center",
            padding: "0 8px", gap: "6px",
          }}
        >
          <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: HEALTH_COLOR[d.health], flexShrink: 0 }} />
...
export function ServiceCatalog({ onTriggerOrchestrator }: Props) {
  ...
  const [selectedId, setSelectedId] = useState<string>(
    CATALOG_SERVICES.find(s => s.health === "degraded")?.id ?? CATALOG_SERVICES[0].id
  );

  const filtered = healthFilter === "all"
    ? CATALOG_SERVICES
    : CATALOG_SERVICES.filter(s => s.health === healthFilter);

  const svc = CATALOG_SERVICES.find(s => s.id === selectedId) ?? CATALOG_SERVICES[0];
  const relatedIncidents = INCIDENTS.filter(i => svc.recentIncidents.includes(i.id));
          ...
          {(["all", "healthy", "degraded", "down"] as HealthFilter[]).map(f => (
            <button
              ...
              style={{
                ...
                background: healthFilter === f ? `${HEALTH_COLOR[f as ServiceHealth] ?? "#10b981"}15` : "transparent",
                color: healthFilter === f ? (HEALTH_COLOR[f as ServiceHealth] ?? "#10b981") : "#555",
                borderColor: healthFilter === f ? `${HEALTH_COLOR[f as ServiceHealth] ?? "#10b981"}40` : "#1a1a1a",
          ...
          {filtered.map(s => (
            <button
              ...
              style={{
                ...
                borderLeft: selectedId === s.id ? `2px solid ${HEALTH_COLOR[s.health]}` : "2px solid transparent",
              }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: "7px", marginBottom: "4px" }}>
                <div style={{
                  width: "7px", height: "7px", borderRadius: "50%",
                  background: HEALTH_COLOR[s.health], flexShrink: 0,
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", marginBottom: "6px" }}>
              ...
              <span style={{
                ...
                color: HEALTH_COLOR[svc.health], background: `${HEALTH_COLOR[svc.health]}15`,
                border: `1px solid ${HEALTH_COLOR[svc.health]}40`, padding: "2px 8px", borderRadius: "10px",
              }}>
                {svc.health}
              </span>
              ...
              {relatedIncidents.length === 0 ? (
                ...
                relatedIncidents.map(inc => (
                  <div key={inc.id} style={{ display: "flex", alignItems: "center", gap: "8px", padding: "6px 0", borderBottom: "1px solid #111" }}>
                    <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: inc.status === "active" ? "#ef4444" : inc.status === "investigating" ? "#f59e0b" : "#10b981", flexShrink: 0 }} />
                    <span style={{ fontSize: "10px", fontFamily: "monospace", color: "#555" }}>{inc.id}</span>
                    <span style={{ fontSize: "11px", color: "#888", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{inc.title}</span>
                    <span style={{ fontSize: "10px", color: "#444" }}>{inc.duration}</span>
                  </div>
                ))
              )}
            </div>
