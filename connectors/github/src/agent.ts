...
const TOOLS: ConnectorTool[] = [
  {
    ...
    execute: async (params, creds) => {
      ...
      if (!token) return { prs: [{ id: 0, title: 'No token configured', state: 'error', author: '', mergedAt: null, sha: '' }] }
      const res = await fetch(`https://api.github.com/repos/${params.repo}/pulls?state=${params.state ?? 'open'}&per_page=${params.limit ?? 10}`, {
  ...
  },
  {
    ...
    execute: async (params, creds) => {
      ...
      const res = await fetch(`https://api.github.com/repos/${params.repo}/commits?per_page=${params.limit ?? 10}`, {
        headers: { Authorization: `Bearer ${token}`, Accept: 'application/vnd.github.v3+json' },
      })
  ...
  },
  {
    ...
    execute: async (params, creds) => {
      ...
      const res = await fetch(`https://api.github.com/repos/${params.repo}/contents/${params.path}${ref}`, {
        headers: { Authorization: `Bearer ${token}`, Accept: 'application/vnd.github.v3+json' },
      })
      if (!res.ok) return { content: '', error: `GitHub API ${res.status}` }
