...
export class NotionBootstrap implements IConnectorBootstrap {
  ...
  async bootstrap(tenantId: TenantId, _connectorId: string, payload: Record<string, unknown>): Promise<ConnectorBootstrapResult> {
    ...
    try {
      ...
      if (!res.ok) return { entitiesUpserted: 0, relationshipsUpserted: 0, episodeHints: ['Notion bootstrap: API call failed'] }
      ...
      return { entitiesUpserted: 0, relationshipsUpserted: 0, episodeHints: ['Notion bootstrap: connection failed'] }
    }
  }
}
