...
app.get('/health', (_r, res) => res.json({status:'ok',service:SERVICE}));
app.get('/metrics', (_r, res) => {
  res.set('Content-Type','text/plain');
  res.send(['http_requests_total{service="'+SERVICE+'",method="POST",status_code="200"} 89','http_requests_total{service="'+SERVICE+'",method="POST",status_code="500"} 8','http_request_duration_seconds{service="'+SERVICE+'",quantile="0.5"} 0.045','http_request_duration_seconds{service="'+SERVICE+'",quantile="0.99"} 0.312','error_rate{service="'+SERVICE+'"} 0.08'].join('\n'));
});
app.post('/login', (req, res) => {
  if (slowMode) { const d = 500+Math.random()*2000; setTimeout(() => res.json({status:'ok'}), d); return; }
  console.log(JSON.stringify({level:'info',service:SERVICE,msg:'login',user:req.body?.email}));
  if (Math.random()<0.08) return res.status(500).json({error:'auth_failed'});
