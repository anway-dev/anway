...
describe('vault conformance', () => {
  ...
  it('bootstrap returns valid result structure', async () => {
    ...
    // server is reachable at the default localhost:8200. Each failed fetch
    // attempt itself takes real wall-clock time under load (TCP connect
    // timeout), on top of the 4 real 1s backoff sleeps between attempts —
    // 10s wasn't enough headroom on a loaded machine (observed 10.08s).
  }, 20_000)
})
