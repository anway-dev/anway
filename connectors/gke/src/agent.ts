...
function gkeAuth(creds: ConnectorCreds): { endpoint: string; token: string } {
...
const TOOLS: ConnectorTool[] = [
  {
    ...
    execute: async (params, creds) => {
      const { endpoint, token } = gkeAuth(creds as ConnectorCreds)
      ...
      if (!res.ok) throw new Error(`GKE get_pods failed: HTTP ${res.status}`)
  ...
  },
  {
    ...
    execute: async (params, creds) => {
      ...
      if (!res.ok) throw new Error(`GKE get_deployments failed: HTTP ${res.status}`)
  ...
  },
  {
    ...
    execute: async (params, creds) => {
      ...
      if (!res.ok) throw new Error(`GKE get_pod_logs failed: HTTP ${res.status}`)
  ...
  },
  {
    ...
    execute: async (params, creds) => {
      ...
      if (!res.ok) throw new Error(`GKE get_events failed: HTTP ${res.status}`)
