...
import {
  CLOUD_PROVIDERS, CLOUD_RESOURCES, CLOUD_SECURITY, CLOUD_CONFIG,
...
function ResourceRow({ r, onDebug }: { r: CloudResource; onDebug: (msg: string) => void }) {
  const isHot = r.metrics && (
    (r.metrics.cpu ?? 0) >= 70 ||
    (r.metrics.memory ?? 0) >= 80 ||
    (r.metrics.connections ?? 0) >= 85
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: STATUS_COLOR[r.status], flexShrink: 0 }} />
      ...
      {isHot && (
        <button
          ...
          style={{
            ...
            fontSize: "10px", cursor: "pointer", flexShrink: 0, fontWeight: 600,
          }}
        >
          Debug ✦
        </button>
  ...
  return (
    <div style={{
      ...
      >
        ...
        <span style={{
          fontSize: "9px", fontWeight: 700, color: SEV_COLOR[f.severity],
          background: `${SEV_COLOR[f.severity]}15`, border: `1px solid ${SEV_COLOR[f.severity]}30`,
          borderRadius: "3px", padding: "1px 6px", flexShrink: 0, textTransform: "uppercase",
        }}>
          {f.severity}
        </span>
        <span style={{ fontSize: "9px", color: "#555", textTransform: "capitalize", flexShrink: 0, minWidth: "80px" }}>
          {f.category}
        </span>
      ...
      {expanded && (
        <div style={{ padding: "0 14px 12px", borderTop: "1px solid #111" }}>
          <div style={{ paddingTop: "10px", fontSize: "11px", color: "#888", lineHeight: "1.7", marginBottom: "10px" }}>
            {f.detail}
          </div>
          <button
  ...
  return (
    <div style={{
      ...
      >
        ...
        <span style={{
          fontSize: "9px", fontWeight: 700, color: SEV_COLOR[c.severity],
          background: `${SEV_COLOR[c.severity]}15`, border: `1px solid ${SEV_COLOR[c.severity]}30`,
          borderRadius: "3px", padding: "1px 6px", flexShrink: 0, textTransform: "uppercase",
        }}>
          {c.severity}
        </span>
      ...
      {expanded && (
        <div style={{ padding: "0 14px 12px", borderTop: "1px solid #111" }}>
          ...
          <button
            onClick={() => onDebug(`Config issue: "${c.issue}" on ${c.resource} (${c.service}). Explain why this matters, what breaks if left unfixed, and give me the exact CLI commands or console steps to remediate.`)}
            style={{
              background: "rgba(16,185,129,0.08)", border: "1px solid rgba(16,185,129,0.25)",
              color: "#10b981", padding: "4px 10px", borderRadius: "4px",
              fontSize: "10px", cursor: "pointer", fontWeight: 600,
            }}
          >
            Why + debug steps ✦
...
export function CloudView({ onTriggerOrchestrator }: {
  ...
  const pSummary = CLOUD_PROVIDERS.find(p => p.provider === provider)!;
  const resources = CLOUD_RESOURCES.filter(r => r.provider === provider);
  const security = CLOUD_SECURITY.filter(f => f.provider === provider);
  const config = CLOUD_CONFIG.filter(c => c.provider === provider);
          ...
          {CLOUD_PROVIDERS.map(p => (
            <button
              ...
              style={{
                ...
                padding: "10px 10px", borderRadius: "6px", border: provider === p.provider ? `1px solid ${p.color}33` : "1px solid #1a1a1a",
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: "12px", color: "#d1d5db", fontWeight: 500 }}>{p.label.split(" ").slice(-1)[0] === "Services" ? "AWS" : p.label.split(" ")[0] === "Microsoft" ? "Azure" : p.label.split(" ")[0]}</div>
                {p.connected ? (
      ...
      {!pSummary.connected ? (
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{ padding: "16px 24px", borderBottom: "1px solid #1a1a1a", display: "flex", alignItems: "center", gap: "24px" }}>
            <div>
              <div style={{ fontSize: "11px", color: "#555", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "2px" }}>Amazon Web Services</div>
            ...
            <div style={{ display: "flex", gap: "8px", marginLeft: "auto" }}>
              ...
              <button
                onClick={() => handleDebug("Give me a full cloud health summary for AWS: what's at risk, what's about to break, top security concerns, and recommended priority actions.")}
                style={{
            ...
            {([
              ...
              >
                ...
                {count > 0 && (
                  <span style={{
                    fontSize: "10px",
                    background: id === "security" && criticalSec > 0 ? "rgba(239,68,68,0.15)"
                      : id === "capacity" && hotResources.length > 0 ? "rgba(239,68,68,0.15)"
                      : "#1a1a1a",
                    color: id === "security" && criticalSec > 0 ? "#ef4444"
                      : id === "capacity" && hotResources.length > 0 ? "#ef4444"
                      : "#555",
          ...
          <div style={{ flex: 1, overflowY: "auto" }}>
            ...
            {tab === "overview" && (
              <div>
                ...
                <div style={{ margin: "0 24px", background: "#0e0e0e", border: "1px solid #1a1a1a", borderRadius: "8px", overflow: "hidden" }}>
                  ...
                  {resources.map(r => (
                    <ResourceRow key={r.id} r={r} onDebug={q => handleDebug(q)} />
                  ))}
                </div>
            ...
            {tab === "security" && (
              <div style={{ padding: "16px 24px" }}>
                ...
                {security.map(f => (
                  <SecurityRow key={f.id} f={f} onDebug={q => handleDebug(q)} />
                ))}
              </div>
            )}
            ...
            {tab === "config" && (
              <div style={{ padding: "16px 24px" }}>
                <div style={{ fontSize: "12px", color: "#555", marginBottom: "14px", lineHeight: "1.6" }}>
                  Misconfigurations that don&apos;t cause immediate failure but increase risk, cost, or blast radius over time.
                </div>
                {config.map(c => (
                  <ConfigRow key={c.id} c={c} onDebug={q => handleDebug(q)} />
                ))}
              </div>
            )}
          </div>
