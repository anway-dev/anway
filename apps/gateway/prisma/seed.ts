...
async function seedDemo() {
  ...
  await prisma.$executeRaw`
    INSERT INTO connector_config (tenant_id, connector_type, instance_name, enabled, credentials_enc)
    VALUES
      (${DEMO_TENANT_ID}::uuid, 'prometheus',   'prometheus',   true, NULL),
      (${DEMO_TENANT_ID}::uuid, 'alertmanager', 'alertmanager', true, NULL),
      (${DEMO_TENANT_ID}::uuid, 'loki',         'loki',         true, NULL),
      (${DEMO_TENANT_ID}::uuid, 'grafana',      'grafana',      true, NULL),
      (${DEMO_TENANT_ID}::uuid, 'github',       'github',       true, NULL),
