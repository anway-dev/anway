...
const TIMELINE_COLOR: Record<IncidentTimelineEvent["type"], string> = {
  alert:    "#ef4444",
  deploy:   "#3b82f6",
  spike:    "#f59e0b",
  ack:      "#10b981",
...
const METRIC_STATUS_COLOR: Record<IncidentMetric["status"], string> = {
  critical: "#ef4444",
  warning:  "#f59e0b",
  ok:       "#10b981",
};

const DEPLOY_STATUS_STYLE: Record<IncidentDeploy["status"], { bg: string; color: string; border: string }> = {
...
function Sparkline({ spark, status }: { spark: number[]; status: IncidentMetric["status"] }) {
...
function TimelineStrip({ events }: { events: IncidentTimelineEvent[] }) {
  return (
        ...
        {events.map((evt, i) => {
          ...
          return (
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: "110px" }}>
              ...
              {/* label below */}
              <span style={{
                fontSize: "9px", color: "#888", marginTop: "4px",
                maxWidth: "96px", textAlign: "center", lineHeight: "1.3",
...
function MetricCard({ m }: { m: IncidentMetric }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "8px" }}>
        <span style={{ fontSize: "10px", color: "#888", textTransform: "uppercase", letterSpacing: "0.06em", fontFamily: "monospace" }}>
          {m.name}
        </span>
        <span style={{
          fontSize: "9px", color, background: `${color}18`,
          border: `1px solid ${color}40`, padding: "1px 5px", borderRadius: "3px", fontWeight: 700,
...
function DeployRow({ d }: { d: IncidentDeploy }) {
  const s = DEPLOY_STATUS_STYLE[d.status];
  return (
    <div style={{
      padding: "9px 12px", borderRadius: "6px", marginBottom: "6px",
      background: s.bg, border: `1px solid ${s.border === "transparent" ? "#1a1a1a" : s.border}`,
      borderLeft: d.status === "culprit" ? `3px solid #ef4444` : `1px solid ${s.border === "transparent" ? "#1a1a1a" : s.border}`,
    }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "3px" }}>
        <span style={{ fontFamily: "monospace", fontSize: "11px", color: s.color, fontWeight: 600 }}>{d.sha}</span>
        <span style={{
          fontSize: "9px", color: s.color, background: `${s.color === "#10b981" ? "rgba(16,185,129,0.1)" : `${s.color}18`}`,
          border: `1px solid ${s.color}40`, padding: "1px 5px", borderRadius: "3px", fontWeight: 700, textTransform: "uppercase",
        }}>
          {d.status}
        </span>
      </div>
...
function PRRow({ pr }: { pr: IncidentPR }) {
  const isSuspect = pr.status === "suspect";
  return (
    <div style={{
      padding: "9px 12px", borderRadius: "6px", marginBottom: "6px",
      background: isSuspect ? "rgba(245,158,11,0.06)" : "transparent",
      border: `1px solid ${isSuspect ? "rgba(245,158,11,0.25)" : "#1a1a1a"}`,
    }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "3px" }}>
        <span style={{ fontSize: "11px", color: isSuspect ? "#f59e0b" : "#888", fontFamily: "monospace", fontWeight: 600 }}>
          #{pr.number}
        </span>
        {isSuspect && (
          <span style={{
            fontSize: "9px", color: "#f59e0b", background: "rgba(245,158,11,0.12)",
            border: "1px solid rgba(245,158,11,0.3)", padding: "1px 5px", borderRadius: "3px", fontWeight: 700,
          }}>
            SUSPECT
          </span>
        )}
      </div>
      <div style={{ fontSize: "10px", color: "#d1d5db", marginBottom: "2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
        {pr.title}
      </div>
      <div style={{ fontSize: "10px", color: "#555" }}>
        {pr.author} · {pr.repo} · merged {pr.mergedAt}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ position: "relative", width: "8px", height: "8px", flexShrink: 0 }}>
      ...
      <div style={{
        ...
        background: `${color}30`, animation: "pulse-ring 2s ease-in-out infinite",
      }} />
      <style>{`
...
export function IncidentView({ onTriggerOrchestrator }: {
  ...
  const [selectedId, setSelectedId] = useState<string>(
    INCIDENTS.find(i => i.status === "active")?.id ?? INCIDENTS[0].id
  );
  ...
  return (
    <div style={{ display: "flex", height: "100%", background: "#080808", overflow: "hidden" }}>
      {/* Left panel */}
      <div style={{
        width: "280px", flexShrink: 0, background: "#0a0a0a",
        borderRight: "1px solid #1a1a1a", display: "flex", flexDirection: "column", overflow: "hidden",
      }}>
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: "7px", marginBottom: "5px" }}>
                  ...
                  <span style={{
                    ...
                    background: "#111", border: "1px solid #1a1a1a", padding: "1px 5px", borderRadius: "3px",
                  }}>
                    {inc.id}
                  </span>
                  <span style={{
                    marginLeft: "auto", fontSize: "9px", fontWeight: 700, textTransform: "uppercase",
                    color: STATUS_COLOR[inc.status], background: STATUS_BG[inc.status],
                    border: `1px solid ${STATUS_COLOR[inc.status]}40`, padding: "1px 6px", borderRadius: "10px",
                  }}>
                    {inc.status}
                  </span>
                </div>
                <div style={{
                  fontSize: "12px", color: "#d1d5db", fontWeight: 500, marginBottom: "5px",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                  <span style={{
                    fontSize: "9px", color: "#888", background: "#111", border: "1px solid #2a2a2a",
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        {/* Header bar */}
        <div style={{
          padding: "14px 20px", borderBottom: "1px solid #1a1a1a", background: "#0a0a0a",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
            ...
            <span style={{
              ...
              color: SEV_COLOR[selected.severity], background: `${SEV_COLOR[selected.severity]}15`,
              border: `1px solid ${SEV_COLOR[selected.severity]}40`, padding: "2px 8px", borderRadius: "10px",
            }}>
              {selected.severity}
            </span>
            {/* status badge */}
            <span style={{
              ...
              border: `1px solid ${STATUS_COLOR[selected.status]}40`, padding: "2px 8px", borderRadius: "10px",
            }}>
              {selected.status}
            </span>
            {/* service badge */}
            <span style={{
              fontSize: "10px", color: "#888", background: "#111",
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
              <div style={{
                width: "20px", height: "20px", borderRadius: "50%", background: "#1a2a1a",
                border: "1px solid rgba(16,185,129,0.3)", display: "flex", alignItems: "center",
                justifyContent: "center", fontSize: "8px", color: "#10b981", fontWeight: 700,
              }}>
                {selected.oncall.slice(0, 2).toUpperCase()}
              </div>
            ...
            <button
              ...
              style={{
                ...
                color: "#10b981", padding: "5px 12px", borderRadius: "6px",
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: "16px" }}>
          {/* Hypothesis box */}
          <div style={{
            background: "#0a150e", border: "1px solid rgba(16,185,129,0.2)",
            borderLeft: "3px solid #10b981", borderRadius: "8px", padding: "14px 16px",
            position: "relative",
          }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
              ...
              <span style={{
                fontSize: "10px", color: "#10b981", background: "rgba(16,185,129,0.12)",
                border: "1px solid rgba(16,185,129,0.3)", padding: "2px 8px", borderRadius: "3px", fontWeight: 700,
              }}>
                Confidence: 0.91
              </span>
            </div>
          ...
          <div style={{ background: "#0e0e0e", border: "1px solid #1a1a1a", borderRadius: "8px", padding: "12px 16px" }}>
            <div style={{ fontSize: "10px", color: "#555", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "10px", fontWeight: 600 }}>
              Timeline
            </div>
          ...
          <div style={{ background: "#0e0e0e", border: "1px solid #1a1a1a", borderRadius: "8px", overflow: "hidden" }}>
            <button
              ...
              style={{
                ...
                cursor: "pointer", borderBottom: runbookOpen ? "1px solid #1a1a1a" : "none",
              }}
            >
                ...
                {selected.runbook.map((step, i) => (
                  <div
                    ...
                    style={{
                      ...
                      borderBottom: i < selected.runbook.length - 1 ? "1px solid #111" : "none",
                    }}
                  >
                    <span style={{ fontFamily: "monospace", fontSize: "10px", color: "#10b981", flexShrink: 0, marginTop: "1px" }}>
