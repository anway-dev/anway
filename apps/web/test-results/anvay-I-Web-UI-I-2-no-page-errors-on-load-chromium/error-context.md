...
- Name: anvay.spec.ts >> I: Web UI >> I.2 no page errors on load
- Location: e2e/anvay.spec.ts:210:7
...
> 213 |     await page.goto('http://localhost:3000', { waitUntil: 'networkidle', timeout: 60_000 })
  ...
  307 |     const resp = await request.post(`${GATEWAY}/api/incidents`, {
  308 |       headers: { Authorization: `Bearer ${token}` },
  309 |       data: { title: 'bad', severity: 'invalid' },
  310 |     })
  311 |     expect(resp.status()).toBe(400)
  312 |   })
  313 | 
```