...
// shell string) so that tool-call parameters (LLM-reachable, unlike
// bootstrap.ts's API-response-only values) can never be interpreted as shell
// metacharacters. Matches the safe pattern already used by connectors/k8s's
// agent.ts (spawnSync('kubectl', argsArray, ...)).
// ---------------------------------------------------------------------------
  ...
  // -- get_cloud_metrics ----------------------------------------------------
  {
    definition: {
      ...
      description:
        ...
        'For other services the caller should specify the appropriate dimension via a future namespace/dimension parameter extension.',
      parameters: {
        type: 'object',
        properties: {
...
export class AwsCloudwatchAgent implements IConnectorAgent {
  readonly connectorType = 'aws-cloudwatch'
